...
현재 `WhitespaceMode`로 세 가지 정책 모두 노출 — `'preserve'`(리터럴), `'ignore'`(완전 무시), `'split'`(boundary 분리, 순서 무관 AND). 기본값은 `'ignore'`.

`'split'` 모드는 토큰 간 cross-term 충돌 회피(어느 토큰이 어느 span을 점령할지 joint 최적화)를 의도적으로 안 한다. 첫 토큰에 매치된 텍스트가 두 번째 토큰에도 다시 매치되어 보이는 UX 통증은 실사용에서 빈도 측정 후 별도 노브 도입 여부 재판단. 다만 의도 없는 중복/prefix 토큰 입력(`"안녕 안"`, `"a ab"`)은 atom-prefix dedup으로 자동 흡수되므로 IME journey에서 생기는 redundant 토큰은 부풀림 없이 한 번만 평가된다.

vscode에서는 전체 검색어 trim 후 문자 사이의 공백들은 공백 문자 그대로 리터럴로 처리, 대상 텍스트에 공백과 매치를 시키는 것 같다 (fuzzly의 `'preserve'` 모드).
