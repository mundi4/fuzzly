...
3. 받침까지 입력이 된 글자 => 실제로 받침을 의도하지 않고 그 이 후 글자의 초성을 의도한 것일 수 있음.  
   `"갑"`은 정말로 `"갑"`을 의도한 것이 아니라 `"가방"`을 의도한 것일 수 있다는 뜻.
   혹은 더 나아가서 `"ㅂ"`이 바로 다음 글자가 그냥 그 뒤 어딘가의 아무 글자에 매칭되길 바랬을 수도 있음. 이게 흔한 커맨트팔레트의 기본 동작임.
...
(단순 키 입력 단위 매치라면 `"고"`에서 `"ㄱ"`, `"아"`에서 `"ㅏ"`가 매치 성공함 => wrong)
...
이후 글자들의 초성에도 매치시도를 해봐야 함. > spillover
